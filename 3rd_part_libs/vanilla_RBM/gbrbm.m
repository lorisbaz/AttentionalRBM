...
function[W,c,b] = gbrbm(batchdata,init_params)
eta = init_params.eta;
...
for epoch = 1:maxepoch,
    ...
    for batch = 1:numbatches,
		...
		ph = logistic(data*W/sig + repmat(b,numcases,1));
		...
		negdata = sig*nhstates*W' + repmat(c,numcases,1); % gaussian sampling
		negdatastates = negdata + sig*randn(numcases,numdims);
		
		nh = logistic(negdatastates*W/sig + repmat(b,numcases,1));
		nhstates = nh > rand(numcases,numhid);
		
		dW = (data'*ph - negdatastates'*nh)/sig;
		dc = (sum(data) - sum(negdatastates))/(sig^2);
		db = sum(ph) - sum(nh);
